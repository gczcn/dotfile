...
return {
  {
    ...
    config = function()
      ...
      local on_attach = function(_, bufnr)
        ...
        vim.keymap.set('n', '<M-[>', function() vim.diagnostic.jump({ count = -1 }) end, opts) -- jump to previous diagnostic in buffer

        opts.desc = 'Go to next diagnostic'
        vim.keymap.set('n', '<M-]>', function() vim.diagnostic.jump({ count = 1 }) end, opts) -- jump to next diagnostic in buffer
